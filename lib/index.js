...
\
.creenv-slider-container.vertical {\
  height: 100%;\
  width: 23px;\
  min-height: 40px;\
  cursor: s-resize;\
...
\
.creenv-slider-container.vertical .creenv-slider-progress {\
  width: 100%;\
  height: 0;\
  bottom: 0; top: auto;\
  border-radius: 30px;\
...
\
.creenv-slider-container.vertical .creenv-slider-cursor {\
 ...
 * @param {?boolean} vertical weither the slider needs to be rendered vertically
 */
function Slider (min, max, step, value, onChange, onChangeFinished, vertical) {
  ...
  if (vertical) this.container.classList.add("vertical");
  ...
  this.dom.addEventListener("mousedown", function(event) {
    ...
    that.x = that.vertical ? rect.y : rect.x;
    that.width = that.vertical ? rect.height : rect.width;

    that.clicked = true;
    that.updateValue(that.vertical ? event.pageY : event.pageX);
  });
 ...
 * @private 
 */
Slider.prototype.updateValue = function (mousePos) {
...
Slider.prototype.updateGraphics = function () {
  ...
  if (this.vertical) width = 100-width;
  if (this.vertical) {
    this.progress.style.height = width+"%";
    this.cursor.style.bottom = width+"%";
  } else {
